...
const addCategoryValidation = z.strictObject({
  name: z
    ...
    .nonempty({ error: 'Category is required' })
    .min(3, { error: 'Category must be at least 3 characters long' })
    .max(100, { error: 'Category must be at most 100 characters long' })
    .regex(/^[a-zA-Z0-9 ]+$/, { error: 'Category must contain only letters, numbers, and spaces' }),
});
